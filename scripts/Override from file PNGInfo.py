...
# The returned values are passed to the run method as parameters.
    ...
    def ui(self, is_img2img):
        ...
        addTags = gr.Textbox(label="Tags to add")
        addTagsAtEnd = gr.Textbox(label="Tags to add at the end")
        removeTags = gr.Textbox(label="Tags to remove (Remove only tags already in each image)")
        ...
        return [wantPrompt, addTags, addTagsAtEnd, removeTags, wantNegative, wantSeed,wantSteps,wantCFGScale,wantSize,wantdenoise]
...
# what is returned by the process_images method.
    ...
    def run(self, p, wantPrompt,addTags,addTagsAtEnd,removeTags,wantNegative,wantSeed,wantSteps,wantCFGScale,wantSize,wantdenoise):
        ...
        if wantPrompt and 'Prompt' in deets:
            ...
            if addTags:
                if addTags.endswith(' ') == False and addTags.endswith(',') == False:
                    addTags = addTags + ','
                p.prompt = ''.join([addTags, p.prompt])

            if addTagsAtEnd:
                if addTags.startswith(' ') == False and addTags.startswith(',') == False:
                    addTagsAtEnd = ',' + addTagsAtEnd
                p.prompt = ''.join([p.prompt, addTagsAtEnd])
